...
void UCI::handlePosition(const std::vector<std::string> &tokens) {
  ...
  */
  ...
  for(size_t i = movesIndex + 1; i < tokens.size(); i++) {
    ...
    */
    //cout << "this much ran\n" << endl;
    vector<Move> legalMoves = this -> game.moveHandler -> generateLegalMoves(*this -> game.board);
    // cout << "size of legal moves : " << board -> legalMoves.size() << endl;
    // exit(1);
    bool found = false;
    for(const auto &generatedLegalMove : legalMoves) {
      if(this -> game.moveHandler -> legalMoveToString(generatedLegalMove) == moveStr) {
        // cout << "applying : " << this -> game.moveHandler -> legalMoveToString(generatedLegalMove) << endl;
        this -> game.moveHandler -> applyMove(generatedLegalMove, *this -> game.board);
...
void UCI::searchWorker()
{
    ...
    // handle end state : win, stalemate, three fold, 5 fold, and 50 move draw here, ie, send appropraite response (not compatible with UCI)
    // cout << legalMoves.size() << endl;

    std::string bestMoveStr = "bestmove ";
    this -> game.determineGameState();
    if(this -> game.gameState != Game::GameState::ONGOING) {
      bestMoveStr += "0000";
